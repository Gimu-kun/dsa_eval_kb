  ...
  <script>
    ...
    async function fetchKBData(manualTrigger = false) {
      ...
      const [cData, ontIData, dataIData, rData, ontAData, dataAData, ruData, hData, fData, ontOpData, dataOpData] = await Promise.all([
        ...
        fetchJSONFile(`./ontology/instances.json?_t=${t}`),
        fetchJSONFile(`./data/instances.json?_t=${t}`),
        fetchJSONFile(`./ontology/relations.json?_t=${t}`),
        fetchJSONFile(`./ontology/assertions.json?_t=${t}`),
        fetchJSONFile(`./data/assertions.json?_t=${t}`),
        ...
        fetchJSONFile(`./ontology/operands.json?_t=${t}`),
        fetchJSONFile(`./data/operands.json?_t=${t}`)
      ]);
    ...
    function renderConceptCard(card, item) {
      ...
      if (Array.isArray(item.attributes) && item.attributes.length > 0) {
        attrRowsHtml = item.attributes.map(attr => {
          ...
          const defaultStr = (attr.default !== undefined && attr.default !== null)
            ? ` <span style="font-size: 0.78rem; color: var(--text-sub);">default: <code>${typeof attr.default === 'string' ? attr.default : JSON.stringify(attr.default)}</code></span>`
            : '';
          return `
            <div class="attr-row">
      ...
      if (node.type === 'concept') {
        ...
        if (Array.isArray(data.attributes) && data.attributes.length > 0) {
          ...
          data.attributes.forEach(attr => {
            const defPart = (attr.default !== undefined && attr.default !== null)
              ? ` <span style="font-size: 0.78rem; color: var(--text-sub);">= ${typeof attr.default === 'string' ? attr.default : JSON.stringify(attr.default)}</span>`
              : '';
            html += `<div class="attr-row"><span class="attr-name">${attr.name}</span><span class="attr-type-pill">${attr.value_type}</span>${defPart}</div>`;
          });
          html += '</div>';
        }
      } else {
