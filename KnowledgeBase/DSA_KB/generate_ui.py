...
<body>
  ...
  <script>
    ...
    async function fetchKBData(manualTrigger = false) {
      ...
      const [cData, iData, rData, aData, ruData, hData] = await Promise.all([
        ...
        fetchJSONFile(`./ontology/hierachy.json?_t=${t}`)
      ]);
      ...
      const fingerprint = `${concepts.length}_${hierarchy.length}_${instances.length}_${relations.length}_${assertions.length}_${rules.length}_${JSON.stringify(concepts).length}_${JSON.stringify(instances).length}`;
    ...
    function renderConceptCard(card, item) {
      ...
      if (Array.isArray(item.operation) && item.operation.length > 0) {
        opHtml = `<div style="display: flex; flex-direction: column; gap: 0.35rem; margin-top: 0.35rem;">` + 
          item.operation.map(op => `<div style="font-family: 'JetBrains Mono'; font-size: 0.85rem; color: var(--concept-color);">• ${typeof op === 'string' ? op : JSON.stringify(op)}</div>`).join('') +
          `</div>`;
      }

      let invHtml = '<span style="color: var(--text-muted); font-style: italic;">Không có</span>';
      if (item.invariant && (Array.isArray(item.invariant) ? item.invariant.length > 0 : true)) {
        invHtml = `<div class="text-quote-box">${Array.isArray(item.invariant) ? item.invariant.join('<br>') : item.invariant}</div>`;
      }
